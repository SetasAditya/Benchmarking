...
class Agent(object):
    ...
    def __init__(self, exp_name, env, checkpoint):
        self.device = torch.device("cpu")  # Use CPU instead of CUDA
        ...
        self.tensor_replay_reward = torch.FloatTensor([])  # Use CPU tensors
        self.tensor_replay_policy = torch.FloatTensor([])  # Use CPU tensors
    ...
    def load_checkpoint(self, path):
        ...
        state = torch.load(path, map_location="cpu")  # Load on CPU
    ...
    def exploration_rand(self, n_explore):
        ...
        pi_explore = pi - self.epsilon * rand_sign * torch.FloatTensor(n_explore-1, self.action_space).uniform_()
